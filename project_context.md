...
│   │   ├── src/query-match.ts       # Boolean query matcher; whole-word, spacing-preserving
...
| P3.6 | Deterministic pre-filter — whole-word match, negative keywords, domain blocklist | A3 | 1 h | **TODO — now the highest-value task in P3.** Google News ignores our boolean query semantics (see 0.8.5), so precision comes from here, not from the query builder. |
...
| M3 Collection working | P3 | 🟡 P3.1–P3.3 done (68 collector tests); Google News **live-verified**, GDELT still rate-limited |
