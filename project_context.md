...
| AD-33 | **A fixture run never writes to the production database.** When `fixture` is among the providers and `DB_PATH` is not set explicitly, the writing CLIs default to `./data/dev.sqlite` | **ACCEPTED** | The fixture corpus is hand-authored — invented headlines. During P7, scheduler and smoke tests run with `--providers fixture` inserted **6 fabricated articles into `data/press.sqlite`**, a committed deliverable, where the dashboard cannot distinguish them from real coverage. Nothing failed and no test caught it; the fresh-clone rehearsal did. An explicit `DB_PATH` still wins, so the guard changes only the default. |
| AD-34 | **Every writing process checkpoints the WAL before closing** (`PRAGMA wal_checkpoint(TRUNCATE)`) | **ACCEPTED** | `data/press.sqlite` is a committed deliverable, and SQLite in WAL mode keeps recent writes in a `-wal` sidecar that is gitignored as transient. Without an explicit checkpoint the committed `.sqlite` is **silently stale**: `git status` reports nothing, `git hash-object` matches HEAD exactly, and a fresh clone reads different data from the machine that produced it. 4.8 MB of writes had accumulated outside the committed file before the rehearsal caught it. |
...
│   ├── spot-check.json              # 24 hand-reviewed production mentions (R13, P8.2)
